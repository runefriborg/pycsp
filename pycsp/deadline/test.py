...
class DeadlineTestCase(unittest.TestCase):
    ...
    def test_Channel_priority(self):
        @process
        def source(idx, chan_out, deadline):
            if deadline :
                Set_deadline(deadline)
            Wait(self.deadline)
            Remove_deadline()
        ...
        @process
        def test(chan, priority):
            self.assertTrue(chan.priority() == priority)
        
        chan = Channel()
        lowest = source(2,-chan,None)
        Set_deadline(2*self.deadline,lowest)        
        testprocess = test(chan,Get_deadline(lowest))
        Parallel(
            source(1,-chan, 3*self.deadline),
            lowest,
            source(3,-chan, None),
            testprocess
        )
